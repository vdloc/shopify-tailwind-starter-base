    ...
    if (
      ...
      if (t.slidesPerView === 'auto') {
        ...
        if (
          ...
          A = s.isHorizontal() ? Ge(I, 'width') : Ge(I, 'height');
        else {
          const $ = e(C, 'width'),
            H = e(C, 'padding-left'),
            M = e(C, 'padding-right'),
            L = e(C, 'margin-left'),
            G = e(C, 'margin-right'),
            X = C.getPropertyValue('box-sizing');
          if (X && X === 'border-box') A = $ + L + G;
          else {
...
function St(s) {
  ...
  for (let n = 0; n < i.length; n += 1) {
    ...
    w && (e.visibleSlides.push(u), e.visibleSlidesIndexes.push(n)),
      qe(u, w, t.slideVisibleClass),
      qe(u, h, t.slideFullyVisibleClass),
      (u.progress = a ? -l : l),
...
function xt() {
  ...
  n &&
    (f ||
      ((o = dt(n, `.${t.slideClass}, swiper-slide`)[0]),
      t.loop && !o && (o = e[0]),
      (u = ot(n, `.${t.slideClass}, swiper-slide`)[0]),
  ...
  if (
    ...
    if (e === 0) c[l ? 'scrollLeft' : 'scrollTop'] = -o;
      ...
      if (!r.support.smoothScroll)
        return (
          Qe({ swiper: r, targetPosition: -o, side: l ? 'left' : 'top' }), !0
        );
...
function kt(s, e) {
  ...
  i.cssMode ||
    ...
    tt({ swiper: t, runCallbacks: s, direction: e, step: 'Start' }));
}
function Gt(s, e) {
  s === void 0 && (s = !0);
  const t = this,
  ...
  (t.animating = !1),
    !i.cssMode &&
      (t.setTransition(0),
      tt({ swiper: t, runCallbacks: s, direction: e, step: 'End' }));
}
var Ht = { setTransition: Ot, transitionStart: kt, transitionEnd: Gt };
function Bt(s, e, t, i, a) {
  ...
  if (c.cssMode) {
    ...
    if (e === 0)
      ...
      if (!r.support.smoothScroll)
        return (
          Qe({ swiper: r, targetPosition: z, side: E ? 'left' : 'top' }), !0
        );
...
var Ut = { loopCreate: qt, loopFix: _t, loopDestroy: jt };
function Kt(s) {
  const e = this;
  if (
...
var Qt = { setGrabCursor: Kt, unsetGrabCursor: Zt };
function Jt(s, e) {
  e === void 0 && (e = this);
  function t(i) {
...
function es(s) {
  ...
  let n = i.target;
  if (
    (r.touchEventsTarget === 'wrapper' && !lt(n, e.wrapperEl)) ||
...
const st = (s, e) => {
  ...
  !a ||
    ...
    i.updateOnWindowResize
      ...
      : s[u]('observerUpdate', je, !0),
    a[n]('load', s.onLoad, { capture: !0 }));
};
function ls() {
  const s = this,
    { params: e } = s;
  (s.onTouchStart = es.bind(s)),
    (s.onTouchMove = ts.bind(s)),
    (s.onTouchEnd = ss.bind(s)),
    (s.onDocumentTouchStart = ns.bind(s)),
    e.cssMode && (s.onScroll = rs.bind(s)),
    (s.onClick = is.bind(s)),
    (s.onLoad = as.bind(s)),
    st(s, 'on');
}
...
var ds = { attachEvents: ls, detachEvents: os };
...
var us = { setBreakpoint: cs, getBreakpoint: fs };
function ps(s, e) {
  const t = [];
  return (
    s.forEach((i) => {
...
var gs = { addClasses: ms, removeClasses: hs };
function vs() {
  const s = this,
  ...
  destroy(e, t) {
    ...
    return (
      typeof i.params > 'u' ||
        ...
        e !== !1 &&
          (i.el && typeof i.el != 'string' && (i.el.swiper = null), rt(i)),
        (i.destroyed = !0)),
      null
    );
  }
...
function bs(s) {
  ...
  function u(g, h) {
    ...
    const { addSlidesBefore: E, addSlidesAfter: z } = e.params.virtual,
      { from: D, to: A, slides: T, slidesGrid: P, offset: I } = e.virtual;
    ...
    if (e.params.virtual.renderExternal) {
      e.params.virtual.renderExternal.call(e, {
        ...
        slides: (function () {
          const Y = [];
          for (let _ = H; _ <= M; _ += 1) Y.push(T[_]);
          return Y;
        })(),
      }),
    ...
    const X = [],
      ...
      k = (R) => {
        let Y = R;
        return (
          R < 0 ? (Y = T.length + R) : Y >= T.length && (Y = Y - T.length), Y
        );
      };
    if (g)
      e.slides
...
function Es(s) {
  ...
  function o(p) {
    ...
    return (
      ...
      p.shiftKey && !T && ((T = P), (P = 0)),
      (T || P) &&
        p.deltaMode &&
        (p.deltaMode === 1 ? ((T *= 40), (P *= 40)) : ((T *= 800), (P *= 800))),
      T && !D && (D = T < 1 ? -1 : 1),
      P && !A && (A = P < 1 ? -1 : 1),
      { spinX: D, spinY: A, pixelX: T, pixelY: P }
    );
  }
  function l() {
  ...
  function h(p) {
    ...
    if (T === 0) return !0;
    z.invert && (T = -T);
    let C = e.getTranslate() + T * z.sensitivity;
    if (
      ...
      const S = { time: Z(), delta: Math.abs(T), direction: Math.sign(T) },
        x =
          n &&
          S.time < n.time + 500 &&
          S.delta <= n.delta &&
      ...
      if (!x) {
        n = void 0;
        let $ = e.getTranslate() + T * z.sensitivity;
        ...
        if (
          ...
          e.updateActiveIndex(),
          e.updateSlidesClasses(),
          ((!H && e.isBeginning) || (!M && e.isEnd)) && e.updateSlidesClasses(),
          e.params.loop &&
            e.loopFix({
...
function Ts(s) {
  ...
  function d() {
    ...
    if (
      ...
      if (
        (p.dynamicBullets &&
          ((f = Ge(T[0], e.isHorizontal() ? 'width' : 'height')),
          b.forEach((S) => {
          ...
          p.dynamicMainBullets > 1 &&
            ...
            c > p.dynamicMainBullets - 1
              ...
              : c < 0 && (c = 0)),
          (P = Math.max(E - c, 0)),
          (I = P + (Math.min(T.length, p.dynamicMainBullets) - 1)),
          (C = (I + P) / 2)),
        T.forEach((S) => {
          const x = [
        ...
        if (
          ...
          for (let H = P; H <= I; H += 1)
            T[H] &&
              T[H].classList.add(...`${p.bulletActiveClass}-main`.split(' '));
          u(x, 'prev'), u($, 'next');
        }
      }
      if (p.dynamicBullets) {
    ...
    b.forEach((T, P) => {
      if (
        (p.type === 'fraction' &&
          (T.querySelectorAll(J(p.currentClass)).forEach((I) => {
        ...
        I === 'horizontal' ? (S = C) : (x = C),
          T.querySelectorAll(J(p.progressbarFillClass)).forEach(($) => {
      ...
      p.type === 'custom' && p.renderCustom
        ? ((T.innerHTML = p.renderCustom(e, E + 1, A)),
          P === 0 && a('paginationRender', T))
        : (P === 0 && a('paginationRender', T), a('paginationUpdate', T)),
        e.params.watchOverflow &&
          e.enabled &&
...
function Ps(s) {
  ...
  function $(O) {
    ...
    let N, R, Y, _, ne, ge, Q, se, Xe, Ve, Ye, Ne, ve, we, Te, Me, Ce, Pe;
    ...
    (k.scale = de || Fe),
      ...
      O && !(f === 1 && de)
        ? ((Ce = l.slideEl.offsetWidth),
          ...
          (ne = Y + Ce / 2 - N),
          (ge = _ + Pe / 2 - R),
          (Xe = l.imageEl.offsetWidth || l.imageEl.clientWidth),
          (Ve = l.imageEl.offsetHeight || l.imageEl.clientHeight),
          (Ye = Xe * k.scale),
          (Ne = Ve * k.scale),
          (ve = Math.min(Ce / 2 - Ye / 2, 0)),
          (we = Math.min(Pe / 2 - Ne / 2, 0)),
          (Te = -ve),
          (Me = -we),
          (Q = ne * k.scale),
          (se = ge * k.scale),
  ...
  i('init', () => {
    ...
    Object.assign(e.zoom, { enable: G, disable: X, in: $, out: H, toggle: M });
}
function Ls(s) {
  let { swiper: e, extendParams: t, on: i } = s;
...
function Is(s) {
  ...
  function b(M) {
    ...
    if (
      !(
        e.pagination &&
        e.pagination.el &&
        (G === e.pagination.el || e.pagination.el.contains(M.target)) &&
        !M.target.matches(J(e.params.pagination.bulletClass))
      )
    ) {
      if (e.navigation && e.navigation.prevEl && e.navigation.nextEl) {
        const X = V(e.navigation.prevEl);
        V(e.navigation.nextEl).includes(G) &&
  ...
  const T = (M, L, G) => {
    ...
    P = (M) => {
      f && f !== M.target && !f.contains(M.target) && (r = !0),
        (e.a11y.clicked = !0);
    },
    I = () => {
      (r = !1),
    ...
    $ = () => {
      ...
      M.containerRoleDescriptionMessage &&
        m(L, M.containerRoleDescriptionMessage),
        M.containerMessage && h(L, M.containerMessage),
        M.containerRole && d(L, M.containerRole);
      const G = e.wrapperEl,
        X = M.id || G.getAttribute('id') || `swiper-wrapper-${u(16)}`,
        O = e.params.autoplay && e.params.autoplay.enabled ? 'off' : 'polite';
      w(G, X), y(G, O), x();
      let { nextEl: k, prevEl: B } = e.navigation ? e.navigation : {};
      (k = V(k)),
        (B = V(B)),
        k && k.forEach((R) => T(R, X, M.nextSlideMessage)),
        B && B.forEach((R) => T(R, X, M.prevSlideMessage)),
        D() &&
          V(e.pagination.el).forEach((Y) => {
  ...
  function H() {
    ...
    (M = V(M)),
      (L = V(L)),
      M && M.forEach((X) => X.removeEventListener('keydown', b)),
      L && L.forEach((X) => X.removeEventListener('keydown', b)),
      D() &&
        V(e.pagination.el).forEach((O) => {
...
function $s(s) {
  ...
  const b = () => {
    ...
    T = (L, G) => {
      ...
      if (((e.autoplay.paused = !0), G)) {
        w && (o = e.params.autoplay.delay), (w = !1), X();
        return;
      }
      (o = (o || e.params.autoplay.delay) - (new Date().getTime() - l)),
        !(e.isEnd && o < 0 && !e.params.loop) && (o < 0 && (o = 0), X());
    },
    P = () => {
  ...
  i('init', () => {
    ...
    i('beforeTransitionStart', (L, G, X) => {
      e.destroyed ||
        !e.autoplay.running ||
        (X || !e.params.autoplay.disableOnInteraction ? T(!0, !0) : A());
    }),
    i('sliderFirstMove', () => {
    ...
    Object.assign(e.autoplay, { start: D, stop: A, pause: T, resume: P });
}
function Ds(s) {
  let { swiper: e, extendParams: t, on: i } = s;
  t({
  ...
  oe({
    ...
    setTranslate: () => {
      ...
      for (let D = 0; D < o.length; D += 1) {
        const A = o[D];
        let T = D;
        p && (T = parseInt(A.getAttribute('data-swiper-slide-index'), 10));
        let P = T * 90,
          I = Math.floor(P / 360);
      ...
      if (
        ...
        if (v)
          ...
          const D = Math.abs(b) - Math.floor(Math.abs(b) / 90) * 90,
            ...
            P = y.shadowScale / A,
            I = y.shadowOffset;
          E.style.transform = `scale3d(${T}, 1, ${P}) translate3d(0px, ${
            d / 2 + I
      ...
      setTranslate: () => {
        ...
        for (let y = 0, v = n.length; y < v; y += 1) {
          ...
          if (
            ...
            M || (M = le('coverflow', p, l ? 'left' : 'top')),
              L || (L = le('coverflow', p, l ? 'right' : 'bottom')),
              M && (M.style.opacity = D > 0 ? D : 0),
  ...
  oe({
    ...
    setTranslate: () => {
      ...
      for (let g = 0; g < c.length; g += 1) {
        ...
        const A = b.join(', '),
          T = `rotateX(${m(E[0])}deg) rotateY(${m(E[1])}deg) rotateZ(${m(
            E[2],
          )}deg)`,
          P =
            v < 0
...
class Us {
  constructor({ container: e, options: t = {} }) {
    (this.container = e), (this.options = t), (this.slider = null);
  }
  init() {
    this.slider = new j(this.container, this.options);
  }
}
export { Us as S };
